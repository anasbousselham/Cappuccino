...
uses
  ...
  JvImage, ComCtrls, StdCtrls, JvExExtCtrls;
...
uses Main_frm, Options_frm, Modify_Authentication_frm,
  Locked_frm, Config, Message_frm, ShutDown, StrUtils, DateUtils, information_frm, PassUnit, CappuccinoAbout;
{$R *.dfm}
...
begin
    Left := Screen.WorkAreaWidth - 187;
    Top := Screen.WorkAreaHeight - 179;
end; // UpDatePosition
    ...
    begin
        ...
        begin
          ...
          // if WindowState = wsNormal then
                Caption := 'Temps restant: '+lblTimeLeft.Caption;
          //  else
...
Year1:=FormatDateTime('yy', Year);
pwd:=settings.PwdClose+Year1+IntToStr(Month)+intToStr(Day);
if Assigned(PassForm) then
    begin
      SetForegroundWindow(FindWindow(nil,'Identification'));
    ...
    try
      ...
      if not ((PassForm.ShowModal = mrOK) and (PassForm.edtpass.Text=pwd)) then
        begin
               //     AddToLog('Identification', Dlgpwd.Edit2.Text + ' incorecte.');
                    
                     Exit;
             end;
    finally
     // AddToLog('Mot de passe incorecte', Dlgpwd.Edit2.Text + ' incorecte.');
     // AddToLog('Identification', Dlgpwd.Edit2.Text + ' correcte.');
      FreeAndNil(PassForm);
    end;
   ...
   frmMain.Tag := 1;
    try
        ...
        begin
            ...
            WriteEventInLog('Fermeture du client Cappuccino');
            Close;
        end;
    except
    end;
...
procedure TfrmSessionOpen.actInformationExecute(Sender: TObject);
begin
frmInformation.Show;
end;

procedure TfrmSessionOpen.Timer1Timer(Sender: TObject);
begin
frmSessionOpen.Show;
Timer1.Enabled:=false;
end;

procedure TfrmSessionOpen.JvImage3Click(Sender: TObject);
begin
actFirstPlant.Execute;
end;

procedure TfrmSessionOpen.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmSessionOpen.actInfoExecute(Sender: TObject);

begin
with TCappuccinoAboutDlg.Create(self) do
try
showmodal
finally
release;
...
procedure TfrmSessionOpen.test1Click(Sender: TObject);
begin
